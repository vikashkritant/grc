...
const LaboratoryTemplate = (props) => {
    ...
    const { processing, details, error, errors, message } = useSelector(state => {
        return {
            processing: state.Pages_Reducers.laboratory.processing,
            details: state.Pages_Reducers.laboratory.details,
            error: state.Pages_Reducers.laboratory.error,
            errors: state.Pages_Reducers.laboratory.errors,
            message: state.Pages_Reducers.laboratory.message

        }
    });

    useEffect(() => {
        dispatch(PAGES_ACTIONS.fetch_laboratory_page_content(props.slug));
    }, [props.slug]);
