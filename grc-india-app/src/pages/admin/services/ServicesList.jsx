...
const ServicesList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.Services_Reducers.list,
            totalPages: state.Services_Reducers.totalPages,
            total: state.Services_Reducers.total,
    ...
    useEffect(() => {
        dispatch(SERVICES_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(SERVICES_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(SERVICES_ACTIONS.fetch_list(filter));
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/services/edit/${row.id}`}>
