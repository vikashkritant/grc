...
const NewInitiativeAdd = (props) => {
    ...
    useEffect(() => {
        dispatch(NEW_INITIATIVES_ACTIONS.fetch_meta_details(1));
    }, []);
    ...
    const { processing, error, errors, message, details, redirectTo } = useSelector(state => {
        return {
            ...
            details: state.NewInitiatives_Reducers.meta_details,
    ...
    const submitForm = (e) => {
        ...
        if (!errorFound) {
            ...
            dispatch(NEW_INITIATIVES_ACTIONS.fetch_meta_update(pageFormFields));//add_upcoming_event
        }
...
export default NewInitiativeAdd;