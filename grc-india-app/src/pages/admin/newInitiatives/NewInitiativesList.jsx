...
const NewInitiativesList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.NewInitiatives_Reducers.list,
            totalPages: state.NewInitiatives_Reducers.totalPages,
            total: state.NewInitiatives_Reducers.total,
    ...
    useEffect(() => {
        dispatch(NEW_INITIATIVES_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(NEW_INITIATIVES_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(NEW_INITIATIVES_ACTIONS.fetch_list(filter));
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/new-initiatives/edit/${row.id}`}>
...
export default NewInitiativesList;