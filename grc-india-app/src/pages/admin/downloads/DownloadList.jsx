...
const DownloadList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.Downloads_Reducers.list,
            totalPages: state.Downloads_Reducers.totalPages,
            total: state.Downloads_Reducers.total,
    ...
    useEffect(() => {
        dispatch(DOWNLOAD_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(DOWNLOAD_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(DOWNLOAD_ACTIONS.fetch_list(filter));
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/downloads/edit/${row.id}`}>
