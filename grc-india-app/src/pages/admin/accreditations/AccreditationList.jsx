...
import * as ACCREDITATIONS_ACTIONS from '../../../actions/admin/accreditationsActions';
...
const AccreditationList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            processing: state.Accreditations_Reducers.processing,
            error: state.Accreditations_Reducers.error,
            errors: state.Accreditations_Reducers.errors,
            message: state.Accreditations_Reducers.message,
            list: state.Accreditations_Reducers.list,
            totalPages: state.Accreditations_Reducers.totalPages,
            total: state.Accreditations_Reducers.total,
            redirectTo: state.Accreditations_Reducers.redirectTo
        }
    });
    ...
    useEffect(() => {
        dispatch(ACCREDITATIONS_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(ACCREDITATIONS_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(ACCREDITATIONS_ACTIONS.fetch_list(filter));
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/accreditations/edit/${row.id}`}>
