...
import { titleToSlug } from '../../../utils/html_util';
...
const ProjectEdit = (props) => {
    ...
    useEffect(() => {
        dispatch(PROJECTS_ACTIONS.fetch_details(id));
        dispatch(UTILS_ACTIONS.fetch_states());
    }, [id]);

    const { processing, errors, details, states, redirectTo } = useSelector(state => {
        return {
    ...
    const submitForm = (e) => {
        ...
        if (!errorFound) {
            ...
            dispatch(PROJECTS_ACTIONS.fetch_update(pageFormFields));//add_upcoming_event
        }
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={12}>
                            <TextField
                                ...
                                onBlur={(e) => setPageForm({ ...pageForm, slug: titleToSlug(e.currentTarget.value), slugError: '' })}
