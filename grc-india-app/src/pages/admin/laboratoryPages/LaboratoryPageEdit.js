...
import * as LABORATORY_ACTIONS from '../../../actions/admin/laboratoryActions';
...
const LaboratoryPageEdit = (props) => {
    ...
    useEffect(() => {
        dispatch(LABORATORY_ACTIONS.fetch_details(id));
    }, [id]);

    const { processing, error, errors, message, details, redirectTo } = useSelector(state => {
        return {
            processing: state.Laboratory_Reducers.processing,
            error: state.Laboratory_Reducers.error,
            errors: state.Laboratory_Reducers.errors,
            message: state.Laboratory_Reducers.message,
            details: state.Laboratory_Reducers.details,
            redirectTo: state.Laboratory_Reducers.redirectTo
        }
    });
    ...
    const submitForm = (e) => {
        ...
        if (!errorFound) {
            ...
            dispatch(LABORATORY_ACTIONS.fetch_update(pageFormFields));//add_upcoming_event
        }
...
export default LaboratoryPageEdit;