...
const ClienteleList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.Clienteles_Reducers.list,
            totalPages: state.Clienteles_Reducers.totalPages,
            total: state.Clienteles_Reducers.total,
    ...
    const openClientProjectList = (e, clientId) => {
        navigate(`/admin/client/${clientId}/projects/list`);
    }
    ...
    return <Fragment>
        ...
        <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="center">
                                        {
                                            row.total_projects > 0 ? <Chip variant="fill" label={row.total_projects} color="success" icon={<AddIcon />} onClick={(e) => openClientProjectList(e, row.id)} /> : <Chip variant="fill" label={row.total_projects} color="error" icon={<AddIcon />} onClick={(e) => openClientProjectList(e, row.id)} />
                                        }
                                    </TableCell>
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/clienteles/edit/${row.id}`}>
                ...
                <TableFooter>
                    <CustomTablePagination colSpan={6} handleChangePage={handleChangePage} handleChangeRowsPerPage={handleChangeRowsPerPage} itemPerPage={filter.itemPerPage} page={filter.page} total={total} />
                </TableFooter>
            </Table>
        </TableContainer>
    </Fragment>;
