...
const BannerList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.Banners_Reducers.list,
            totalPages: state.Banners_Reducers.totalPages,
            total: state.Banners_Reducers.total,
    ...
    useEffect(() => {
        dispatch(BANNERS_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(BANNERS_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(BANNERS_ACTIONS.fetch_list(filter));
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell align="left" sortDirection={filter.orderBy === 'heading1' ? filter.order : false}>
                            <TableSortLabel
                                active={true}
                                direction={filter.orderBy === 'heading1' ? filter.order : 'asc'}
                                onClick={() => sortingHandler('heading1')}
                            >
                                Heading1
                            </TableSortLabel>
                        </TableCell>
                        <TableCell align="left" sortDirection={filter.orderBy === 'heading2' ? filter.order : false}>
                            <TableSortLabel
                                active={true}
                                direction={filter.orderBy === 'heading2' ? filter.order : 'asc'}
                                onClick={() => sortingHandler('heading2')}
                            >
                                Heading2
                            </TableSortLabel>
                        </TableCell>
                        <TableCell align="left" sortDirection={filter.orderBy === 'heading3' ? filter.order : false}>
                            <TableSortLabel
                                active={true}
                                direction={filter.orderBy === 'heading3' ? filter.order : 'asc'}
                                onClick={() => sortingHandler('heading3')}
                            >
                                Heading3
                            </TableSortLabel>
                        </TableCell>
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="center">
                                        <img src={row.banner} alt={""} style={{ width: "100px", height: "100px" }} />
                                    </TableCell>

                                    <TableCell align="left">{row.heading1}</TableCell>
                                    <TableCell align="left">{row.heading2}</TableCell>
                                    <TableCell align="left">{row.heading3}</TableCell>
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/banner/edit/${row.id}`}>
