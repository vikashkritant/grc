...
const BannerEdit = (props) => {
    ...
    useEffect(() => {
        dispatch(BANNERS_ACTIONS.fetch_details(id));
    }, [id]);
    ...
    useEffect(() => {
        ...
        if (details.id && !pageForm.dataLoaded) {
            setPageForm({
                ...
                link_slug: details.link_button_slug,
                link_title: details.link_button_title,
    ...
    const submitForm = (e) => {
        ...
        if (!errorFound) {
            ...
            dispatch(BANNERS_ACTIONS.fetch_update(pageFormFields));//add_upcoming_event
        }
