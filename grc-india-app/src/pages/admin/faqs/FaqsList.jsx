...
const FaqsList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.Faqs_Reducers.list,
            totalPages: state.Faqs_Reducers.totalPages,
            total: state.Faqs_Reducers.total,
    ...
    useEffect(() => {
        dispatch(FAQS_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(FAQS_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(FAQS_ACTIONS.fetch_list(filter));
        ...
        <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                <TableHead>
                    <TableRow>
                        <TableCell>#</TableCell>
                        <TableCell align="left" sortDirection={filter.orderBy === 'ques' ? filter.order : false}>
                            <TableSortLabel
                                active={true}
                                direction={filter.orderBy === 'ques' ? filter.order : 'asc'}
                                onClick={() => sortingHandler('ques')}
                            >
                                Question
                            </TableSortLabel>
                        </TableCell>
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/faq/edit/${row.id}`}>
