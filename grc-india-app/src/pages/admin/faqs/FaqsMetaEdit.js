...
const FaqsMetaEdit = (props) => {
    ...
    useEffect(() => {
        dispatch(FAQS_ACTIONS.fetch_meta_details(1));
    }, []);
    ...
    const { processing, error, errors, message, details, redirectTo } = useSelector(state => {
        return {
            ...
            details: state.Faqs_Reducers.metaDetails,
    ...
    const submitForm = (e) => {
        ...
        if (!errorFound) {
            ...
            dispatch(FAQS_ACTIONS.fetch_meta_update(pageFormFields));//add_upcoming_event
        }
