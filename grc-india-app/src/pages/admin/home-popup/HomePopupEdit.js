...
import * as EVENTS_ACTIONS from '../../../actions/admin/eventsActions';
...
const HomePopupEdit = (props) => {
    ...
    useEffect(() => {
        dispatch(EVENTS_ACTIONS.fetch_details());
    }, []);

    const { processing, errors, details, redirectTo } = useSelector(state => {
        return {
            processing: state.Events_Reducers.processing,
            error: state.Events_Reducers.error,
            errors: state.Events_Reducers.errors,
            message: state.Events_Reducers.message,
            details: state.Events_Reducers.details,
            redirectTo: state.Events_Reducers.redirectTo
        }
    });
    ...
    const submitForm = (e) => {
        ...
        // if (pageFormFields.sub_title === "") {
        //     errorFound = true;
        //     pageFormFields.sub_titleError = 'this field is required!';
        // } else {
        //     pageFormFields.sub_titleError = '';
        // }
        ...
        if (!errorFound) {
            ...
            dispatch(EVENTS_ACTIONS.fetch_update(pageFormFields));//add_upcoming_event
        }
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={12}>
                            <TextField
                                ...
                                value={pageForm.sub_title}
                                onChange={(e) => setPageForm({ ...pageForm, sub_title: e.currentTarget.value, sub_titleError: '' })}
                                error={pageForm.sub_titleError ? true : false}
                                helperText={pageForm.sub_titleError}
                            />
                        </Grid>
