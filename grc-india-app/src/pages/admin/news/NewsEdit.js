...
import * as NEWS_ACTIONS from '../../../actions/admin/newsActions';
...
const NewsEdit = (props) => {
    ...
    const inputDownloadFile = useRef(null);
    ...
    useEffect(() => {
        dispatch(NEWS_ACTIONS.fetch_details(id));
    }, [id]);

    const { processing, error, errors, message, details, redirectTo } = useSelector(state => {
        return {
            processing: state.News_Reducers.processing,
            error: state.News_Reducers.error,
            errors: state.News_Reducers.errors,
            message: state.News_Reducers.message,
            details: state.News_Reducers.details,
            redirectTo: state.News_Reducers.redirectTo
        }
    });
    ...
    const resetForm = (e) => {
        ...
        inputBannerFile.current.value = null;
        inputThumbnailFile.current.value = null;
        inputDownloadFile.current.value = null;
        dispatch(NEWS_ACTIONS.fetch_details(id));
    }
    ...
    const submitForm = (e) => {
        ...
        if (!errorFound) {
            ...
            if (inputDownloadFile.current.files.length === 0) {
                pageFormFields.downloadFile = null;
            } else {
                pageFormFields.downloadFile = inputDownloadFile.current.files[0];
            }

            dispatch(NEWS_ACTIONS.fetch_update(pageFormFields));//add_upcoming_event
        }
                ...
                <Box component="form" noValidate sx={{ mt: 2, mx: 2, }}>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={4} sx={{ mt: 0 }}>
                            <Typography component="h1" variant="body2">
                                Downloadable File {details.file_path && <a className="btn btn-link" href={details.file_path} target="_blank">{"(PDF)"}</a>}
                            </Typography>
                            <input className="custom-input" accept=".pdf,.PDF" id="contained-button-file" type="file" ref={inputDownloadFile} />
                            {/*<input accept=".pdf,.PDF" id="contained-button-file" type="file"/>*/}

                            <p className="Mui-error MuiFormHelperText-sizeNormal MuiFormHelperText-contained MuiFormHelperText-filled Mui-required css-1wc848c-MuiFormHelperText-root">{pageForm.downloadError}</p>
