...
const SeminarList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.Seminars_Reducers.list,
            totalPages: state.Seminars_Reducers.totalPages,
            total: state.Seminars_Reducers.total,
    ...
    useEffect(() => {
        dispatch(SEMINARS_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(SEMINARS_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(SEMINARS_ACTIONS.fetch_list(filter));
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell align="left" sortDirection={filter.orderBy === 'entry_type' ? filter.order : false}>
                            <TableSortLabel
                                active={true}
                                direction={filter.orderBy === 'entry_type' ? filter.order : 'asc'}
                                onClick={() => sortingHandler('entry_type')}
                            >
                                Type
                            </TableSortLabel>
                        </TableCell>
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">{row.entry_type}</TableCell>
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/seminars/edit/${row.id}`}>
