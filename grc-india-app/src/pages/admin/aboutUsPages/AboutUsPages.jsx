...
const AboutUsPages = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            ...
            list: state.AboutUsPages_Reducers.list,
            totalPages: state.AboutUsPages_Reducers.totalPages,
            total: state.AboutUsPages_Reducers.total,
    ...
    useEffect(() => {
        dispatch(ABOUT_US_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async(id, status) => {
        ...
        await dispatch(ABOUT_US_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(ABOUT_US_ACTIONS.fetch_list(filter));
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/about-us-pages/edit/${row.id}`}>
