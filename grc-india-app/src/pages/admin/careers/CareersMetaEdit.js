...
const CareersMetaEdit = (props) => {
    ...
    useEffect(() => {
        dispatch(CAREERS_ACTIONS.fetch_meta_details(1));
    }, []);
    ...
    const { processing, error, errors, message, details, redirectTo } = useSelector(state => {
        return {
            ...
            details: state.Careers_Reducers.meta_details,
    ...
    const submitForm = (e) => {
        ...
        if (pageFormFields.title === "" || pageFormFields.title === null) {
            errorFound = true;
        ...
        if (!errorFound) {
            ...
            dispatch(CAREERS_ACTIONS.fetch_meta_update(pageFormFields));//add_upcoming_event
        }
...
export default CareersMetaEdit;