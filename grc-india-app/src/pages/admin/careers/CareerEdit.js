...
import * as CAREERS_ACTIONS from '../../../actions/admin/careersActions';
...
const CareerEdit = (props) => {
    ...
    useEffect(() => {
        dispatch(CAREERS_ACTIONS.fetch_details(id));
        dispatch(UTILS_ACTIONS.fetch_states());
    }, [id]);

    const { processing, error, errors, message, details, states, redirectTo } = useSelector(state => {
        return {
            processing: state.Careers_Reducers.processing || state.Utils_Reducers.states.processing,
            error: state.Careers_Reducers.error,
            errors: state.Careers_Reducers.errors,
            message: state.Careers_Reducers.message,
            details: state.Careers_Reducers.details,
            states: state.Utils_Reducers.states.list,
            redirectTo: state.Careers_Reducers.redirectTo
        }
    });
    ...
    const submitForm = (e) => {
        ...
        if (pageFormFields.location === "" || pageFormFields.location === "-1" || pageFormFields.location === null) {
            errorFound = true;
            pageFormFields.locationError = 'this field is required!';
        } else {
            pageFormFields.locationError = '';
        }
        ...
        if (!errorFound) {
            ...
            dispatch(CAREERS_ACTIONS.fetch_update(pageFormFields));//add_upcoming_event
        }
                ...
                <Box component="form" noValidate sx={{ mt: 2, mx: 2, }}>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={3}>
                            <FormControl sx={{ minWidth: "100%" }} error={pageForm.locationError ? true : false}>
                                ...
                                >
                                    <MenuItem value={"-1"}>Select Location</MenuItem>
                                    {
                                        states.map((row, index) => {
                                            return <MenuItem key={`state-location-option-${index}`} value={row.name}>{row.name}</MenuItem>
                                        })
                                    }

                                </Select>
                                <FormHelperText> {pageForm.locationError}</FormHelperText>
                            </FormControl>
                        </Grid>
