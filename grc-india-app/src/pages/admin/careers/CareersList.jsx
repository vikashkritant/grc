...
const CareersList = (props) => {
    ...
    const { processing, error, errors, message, list, totalPages, total, redirectTo } = useSelector(state => {
        return {
            processing: state.Careers_Reducers.processing,
            error: state.Careers_Reducers.error,
            errors: state.Careers_Reducers.errors,
            message: state.Careers_Reducers.message,
            list: state.Careers_Reducers.list,
            totalPages: state.Careers_Reducers.totalPages,
            total: state.Careers_Reducers.total,
    ...
    useEffect(() => {
        dispatch(CAREERS_ACTIONS.fetch_list(filter));
    ...
    const handleConfirmationModalSuccess = async (id, status) => {
        ...
        await dispatch(CAREERS_ACTIONS.fetch_update_status(id, status));
        refreshList();
    };
    const refreshList = () => {
        dispatch(CAREERS_ACTIONS.fetch_list(filter));
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell align="left" sortDirection={filter.orderBy === 'location' ? filter.order : false}>
                            <TableSortLabel
                                active={true}
                                direction={filter.orderBy === 'location' ? filter.order : 'asc'}
                        ...
                        (list.length === 0 && !processing) ?
                            ...
                            list.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        <NavLink to={`/admin/careers/edit/${row.id}`}>
