...
const {
    ...
    update,
    update_status
} = require('../../../controllers/v1/backend/accreditations.js');


routes.get('/list', verifyToken, list);

routes.post('/create', verifyToken, upload.fields([{ name: 'file', maxCount: 1 }]), create);

routes.get('/details/:id', verifyToken, details);

routes.post('/update', verifyToken, upload.fields([{ name: 'file', maxCount: 1 }]), update);
